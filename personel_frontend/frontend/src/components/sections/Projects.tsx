      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-display font-bold text-white uppercase tracking-tight flex items-center gap-4">
            ...
            Directives<span className="text-cyber-purple">_</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          {projects.map((project, index) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className={`group relative glass-panel border border-white/10 p-8 rounded-2xl transition-all duration-500 overflow-hidden ${getColorClass(project.color, 'hover')}`}
            >
              ...
              <div className="relative z-10 flex justify-between items-start mb-8">
                ...
                <div className="flex gap-4">
                  <a href={project.github} className="text-white/40 hover:text-white transition-colors hover:scale-110">
                    <Github className="w-6 h-6" />
                  </a>
                  <a href={project.link} className={`text-white/40 transition-colors hover:scale-110 hover:${getColorClass(project.color, 'text')}`}>
              ...
              <h3 className="text-3xl font-display font-bold text-white mb-4 group-hover:text-white transition-colors tracking-wide">
                {project.title}
              </h3>

              <p className="text-white/60 text-sm font-sans leading-relaxed mb-10 h-[4.5rem]">
                {project.description}
              </p>

              <div className="flex flex-wrap gap-2 mt-auto relative z-10">
                {project.tech.map(t => (
                  <span key={t} className={`px-3 py-1 bg-black/40 border border-white/10 rounded font-mono text-[10px] uppercase tracking-wider text-white/70 group-hover:border-white/30 transition-colors`}>
                    {t}
                  </span>
                ))}
              </div>
